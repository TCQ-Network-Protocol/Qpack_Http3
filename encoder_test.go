...
func BenchmarkEncoder(b *testing.B) {
	b.Run("indexed fields", func(b *testing.B) {
		// Fields that match static table entries exactly
		fields := []HeaderField{
		...
		benchmarkEncoder(b, fields)
	})

	b.Run("literal fields with name reference", func(b *testing.B) {
		// Fields whose name is in the static table, but with a custom value
		fields := []HeaderField{
			{Name: ":status", Value: "666"},
			{Name: "server", Value: "lorem ipsum"},
			{Name: "content-type", Value: "application/octet-stream"},
		}
		benchmarkEncoder(b, fields)
	})

	b.Run("literal fields without name reference", func(b *testing.B) {
		// Fields with custom names not in the static table
		fields := []HeaderField{
			{Name: "x-custom-header", Value: "some-value"},
			{Name: "x-request-id", Value: "abcdef-123456"},
		}
		benchmarkEncoder(b, fields)
	})
...
func benchmarkEncoder(b *testing.B, fields []HeaderField) {
	b.ReportAllocs()
